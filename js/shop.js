...
const App = () => {
    ...
    // 2. FETCH DATA & HANDLE DEEP LINKS
    useEffect(() => {
        const fetchProducts = async () => {
            ...
            if (!error) {
                ...
                // Check if user clicked a link for a specific product
    ...
    useEffect(() => { if (window.lucide) lucide.createIcons(); }, [products, viewingProduct, searchQuery, isMenuOpen, loading]);

    // 3. WHATSAPP LINK GENERATOR
    const handleWhatsApp = (p) => {
        const productLink = `${window.location.origin}${window.location.pathname}?id=${p.id}`;
        ...
        <div className="min-h-screen bg-[#020617] text-white antialiased">
            <header className="sticky top-0 z-50 bg-[#020617]/80 backdrop-blur-xl border-b border-slate-800 px-6 py-4">
                <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
                    <h1 className="text-2xl font-black italic tracking-tighter text-blue-500 uppercase cursor-pointer" onClick={() => {setViewingProduct(null); window.history.pushState({}, '', window.location.pathname);}}>HAZZALE BLUE</h1>
                    <div className="hidden lg:flex items-center gap-2">
                        {categories.map(cat => (
                            <button key={cat} onClick={() => {setActiveCategory(cat); setViewingProduct(null);}}
                                className={`px-5 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all ${activeCategory === cat ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}>
                                {cat}
                            </button>
                        ))}
                    </div>
            ...
            <main className="max-w-7xl mx-auto px-6 py-12">
                {loading ? (
                    ...
                    <div className="animate-in fade-in slide-in-from-bottom-8 duration-500">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 bg-slate-900/50 p-10 rounded-[3rem] border border-slate-800 shadow-2xl">
                            <div className="space-y-4">
                                <div className="bg-white rounded-[2rem] p-10 flex items-center justify-center shadow-inner">
                            ...
                            <div className="flex flex-col justify-center space-y-8">
                                ...
                                <button onClick={() => handleWhatsApp(viewingProduct)} className="w-full py-6 rounded-2xl bg-[#25D366] hover:bg-[#20ba5a] font-black text-sm uppercase flex items-center justify-center gap-3 transition-all shadow-xl shadow-[#25D366]/20">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
                        {filtered.map(p => (
                            <div key={p.id} onClick={() => {setViewingProduct(p); window.history.pushState({}, '', `?id=${p.id}`);}} className="group bg-slate-900/40 rounded-[2.5rem] border border-slate-800/50 p-6 cursor-pointer hover:border-blue-600 transition-all duration-500 shadow-xl">
                                <div className="aspect-square bg-white rounded-3xl mb-8 p-8 flex items-center justify-center overflow-hidden relative">
